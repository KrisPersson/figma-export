    ...
    (acc: string, cur: TParsedColorObject) => {
      if (isRgbaObject(cur.value)) {
        ...
        const parsedKeyAndValue =
          outputFormat === "sass"
            ? `$c-${cur.name.replace(" ", "-")}: ${cssKey?.toLocaleLowerCase()}`
            : `--c-${cur.name.replace(" ", "-")}: var(${cssKey?.toLocaleLowerCase()})`;
    ...
    (variable, i: Number) => {
      ...
      return {
        ...
        value: isVariableAlias(valuePath)
          ? ({ id: valuePath.id, type: valuePath.type } as VariableAlias)
